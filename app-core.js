...
async function loadCategory(key){
  const runs=await q(`monitor_runs?select=*&category_key=eq.${key}&order=collected_at.desc&limit=3`);
...
function decisionCell(categoryKey,x){
  const value=DEVELOPMENT_DECISIONS.get(decisionKey(categoryKey,x.asin))||'';
  return `<select class="development-select" data-category="${esc(categoryKey)}" data-asin="${esc(x.asin)}" aria-label="是否开发 ${esc(x.asin)}"><option value="" ${value?'':'selected'} disabled>请选择</option><option value="是" ${value==='是'?'selected':''}>是</option><option value="否" ${value==='否'?'selected':''}>否</option></select>`;
}
function bindDevelopmentSelectors(){
  document.querySelectorAll('.development-select:not([data-bound])').forEach(el=>{
    el.dataset.bound='1';
    el.addEventListener('change',async()=>{
      const old=DEVELOPMENT_DECISIONS.get(decisionKey(el.dataset.category,el.dataset.asin))||'',value=el.value;
      el.disabled=true;el.classList.add('saving');
      try{if(typeof window.__dashboardPrivateAction!=='function')throw new Error('私有保存通道未就绪');await window.__dashboardPrivateAction('upsert_development_decision',{category_key:el.dataset.category,asin:el.dataset.asin,decision:value});DEVELOPMENT_DECISIONS.set(decisionKey(el.dataset.category,el.dataset.asin),value);el.classList.add('saved');setTimeout(()=>el.classList.remove('saved'),1200)}
      catch(e){el.value=old;alert(e.message||'是否开发保存失败，请重试。')}
      finally{el.disabled=false;el.classList.remove('saving')}
    });
  });
}
async function loadDevelopmentDecisions(){const rows=await q('development_decisions?select=category_key,asin,decision&marketplace=eq.UK');DEVELOPMENT_DECISIONS=new Map((rows||[]).map(x=>[decisionKey(x.category_key,x.asin),x.decision]))}
...
function renderMarketTop(){let rows=[];for(const d of [STORE.party_balloons,STORE.party_packs].filter(Boolean)){rows.push(...d.products.map(x=>({d,x,score:productScore(d,x)})))}rows.sort((a,b)=>b.score-a.score);rows=rows.slice(0,50);$('marketTopNote').textContent=STORE.party_packs?'双类目合并排序 · 最多50个':'当前仅 Party Balloons · 最多50个';$('marketTopRows').innerHTML=rows.length?rows.map(o=>`<tr><td>${categoryTag(o.d.key)}</td><td>${productCell(o.x)}</td><td><span class="score-pill">${o.score}</span></td><td class="rank">#${fmt(o.x.category_rank)}</td><td>${moveCell(o.d,o.x)}</td><td>${fmt(o.x.days_since_launch)}</td><td>${fmt(o.x.review_count)}</td><td>${fmt(o.x.sales_estimate)}</td><td>${esc(opportunityReason(o.d,o.x))}</td><td>${decisionCell(o.d.key,o.x)}</td></tr>`).join(''):'<tr><td colspan="10" class="empty">暂无数据。</td></tr>';bindImgs();bindDevelopmentSelectors()}
...
function renderDetail(key){activeKey=key;const d=STORE[key];const name=key==='party_packs'?'Party Packs':'Party Balloons';$('detailEyebrow').textContent=`Amazon UK · ${name}`;$('detailTitle').textContent=`${name} 类目监控`;if(!d){renderEmptyDetail(name);return}$('detailStatus').textContent='● 动态后台已连接';$('detailStatus').className='status-pill';$('detailTime').textContent=when(d.latest.collected_at);$('detailSummaryText').textContent=`当前有效ASIN ${d.metrics.total} 个，≤100样本 ${d.metrics.top} 个，#200–400潜力池 ${d.metrics.mid} 个，P0机会 ${d.metrics.p0} 个，规则机会指数 ${d.metrics.index}/100。`;
...
function renderDetailTop(d){let rows=d.products.map(x=>({x,score:productScore(d,x)})).sort((a,b)=>b.score-a.score).slice(0,50);$('detailTopNote').textContent=`从 ${d.products.length} 个ASIN筛选，最多显示50个`;$('detailTopRows').innerHTML=rows.length?rows.map(o=>`<tr><td>${productCell(o.x)}</td><td><span class="score-pill">${o.score}</span></td><td>${opportunityType(d,o.x)}</td><td class="rank">#${fmt(o.x.category_rank)}</td><td>${moveCell(d,o.x)}</td><td>${fmt(o.x.days_since_launch)}</td><td>${fmt(o.x.review_count)}</td><td>${fmt(o.x.sales_estimate)}</td><td>${esc(opportunityReason(d,o.x))}</td><td>${decisionCell(d.key,o.x)}</td></tr>`).join(''):'<tr><td colspan="10" class="empty">暂无数据。</td></tr>';bindImgs();bindDevelopmentSelectors()}
