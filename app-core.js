...
function renderDetailTop(d){let rows=d.products.map(x=>({x,score:productScore(d,x)})).sort((a,b)=>b.score-a.score).slice(0,50);$('detailTopNote').textContent=`从 ${d.products.length} 个ASIN筛选，最多显示50个`;$('detailTopRows').innerHTML=rows.length?rows.map(o=>`<tr><td class="development-col">${decisionCell(d.key,o.x)}</td><td>${productCell(o.x)}</td><td><span class="score-pill">${o.score}</span></td><td>${opportunityType(d,o.x)}</td><td class="rank">#${fmt(o.x.category_rank)}</td><td>${moveCell(d,o.x)}</td><td>${fmt(o.x.days_since_launch)}</td><td>${fmt(o.x.review_count)}</td><td>${fmt(o.x.sales_estimate)}</td><td>${esc(opportunityReason(d,o.x))}</td></tr>`).join(''):'<tr><td colspan="10" class="empty">暂无数据。</td></tr>';bindImgs();bindDevelopmentSelectors()}
